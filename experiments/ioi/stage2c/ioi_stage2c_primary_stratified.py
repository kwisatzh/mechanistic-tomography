...
Stage 2b showed that random head-level subsets make the IOI self-repair
interaction a small part of the prediction problem: per-head additive effects
already explain most held-out variance.  Stage 2c runs the single principled
variant motivated by the mechanism: stratify the subset design by primary
Name Mover coverage, because backup heads should matter most when primary
coverage is high.
...
Models:
    ...
    pb_occupancy_interaction:
        additive_head + gamma 1[nP>0]1[nB>0]

    pb_count_interaction:
        additive_head + gamma (nP/|P|)(nB/|B|)

    group_count_interaction:
        additive_head plus P×B, P×E, and B×E count interactions.

Success is gated on paired bootstrap ΔMAE against additive_head, not on a
single split point estimate.
"""
...
def sample_subset_masks(head_records: Sequence[HeadRecord], n_subsets: int, seed: int, p_include: float = 0.35,
    ...
    """Return unique binary masks over heads using primary-stratified sampling.

    This is the Stage-2c design. The Stage-2b uniform random sampler made the
    self-repair interaction a small part of the design, because subsets rarely
    targeted the primary-name-mover group strongly enough for compensation to
    matter. Here we explicitly stratify by the number of primary heads ablated.

    The design still includes clean, singleton, and whole-group anchors, but the
    random part is balanced over primary coverage nP in {0,1,2,3}. Within each
    primary stratum, it samples backup and negative-head counts over a broad
    range. This is the one mechanism-grounded variant: it tests the hypothesis
    that backup effects become important when primary coverage is high.
    """
    rng = np.random.default_rng(seed)
    n = len(head_records)
    groups = {'P': [], 'B': [], 'E': []}
    for i, h in enumerate(head_records):
        groups[h.group].append(i)
    ...
    def make_mask(nP: int, nB: int, nE: int):
        m = np.zeros(n, dtype=int)
        if nP:
            m[rng.choice(groups['P'], size=min(nP, len(groups['P'])), replace=False)] = 1
        if nB:
            m[rng.choice(groups['B'], size=min(nB, len(groups['B'])), replace=False)] = 1
        if nE:
            m[rng.choice(groups['E'], size=min(nE, len(groups['E'])), replace=False)] = 1
        return m

    nP_values = list(range(len(groups['P']) + 1))
    nB_values = [0, 1, 2, 4, 6, len(groups['B'])]
    nB_values = sorted(set(min(x, len(groups['B'])) for x in nB_values))
    nE_values = list(range(len(groups['E']) + 1))

    cells = [(nP, nB, nE) for nP in nP_values for nB in nB_values for nE in nE_values]
    cells.sort(key=lambda t: (-t[0], t[1], t[2]))
    max_attempts = max(10000, n_subsets * 500)
    attempts = 0
    cell_idx = 0
    while len(unique) < n_subsets and attempts < max_attempts:
        attempts += 1
        if attempts <= len(cells) * 4:
            nP, nB, nE = cells[cell_idx % len(cells)]
            cell_idx += 1
        else:
            nP = int(rng.choice(nP_values))
            nB = int(rng.choice(nB_values))
            nE = int(rng.choice(nE_values))
        m = make_mask(nP, nB, nE)
        if m.sum() == 0:
            continue
...
def sample_primary_stratified_masks(head_records: Sequence[HeadRecord], n_subsets: int, seed: int,
                                    include_all_singletons: bool = True,
                                    include_group_subsets: bool = True) -> np.ndarray:
    """Primary-stratified subset design for the IOI self-repair hypothesis.

    Uniform random head subsets under-sample the configurations in which self-repair
    is mechanistically predicted to matter: ablations with high primary-name-mover
    coverage and varying backup coverage.  This design first inserts anchors (clean,
    singletons, whole-group masks) and then fills the budget by stratifying over
    (nP, nB-bin, nE), where nP is exact primary coverage and nB is binned backup
    coverage.  This gives the P×B/count-interaction terms actual leverage without
    exploding to all head pairs.
    """
    rng = np.random.default_rng(seed)
    n = len(head_records)
    groups = {
        'P': [i for i, h in enumerate(head_records) if h.group == 'P'],
        'B': [i for i, h in enumerate(head_records) if h.group == 'B'],
        'E': [i for i, h in enumerate(head_records) if h.group == 'E'],
    }
    masks: List[Tuple[int, ...]] = []

    def add_mask(indices: Sequence[int]):
        m = np.zeros(n, dtype=int)
        for i in indices:
            m[int(i)] = 1
        masks.append(tuple(int(x) for x in m))
            ...
            for B_on in [0, 1]:
                for E_on in [0, 1]:
                    if P_on == B_on == E_on == 0:
                        continue
                    idxs = []
                    if P_on: idxs += groups['P']
                    if B_on: idxs += groups['B']
                    if E_on: idxs += groups['E']
                    add_mask(idxs)

    unique = set(masks)

    # Exact primary counts, backup count bins, exact E counts.  Backup bins are
    # represented as inclusive ranges because |B|=8.
    nP_values = list(range(len(groups['P']) + 1))  # 0..3
    nB_bins = [(0, 0), (1, 1), (2, 3), (4, 5), (6, len(groups['B']))]
    nE_values = list(range(len(groups['E']) + 1))  # 0..2
    cells = [(nP, b0, b1, nE) for nP in nP_values for (b0, b1) in nB_bins for nE in nE_values]

    # Fill cells round-robin so the design has coverage rather than only marginal
    # Bernoulli randomization.  High-primary cells are intentionally revisited more
    # often because the self-repair hypothesis is primary-coverage dependent.
    weighted_cells = []
    for cell in cells:
        nP, b0, b1, nE = cell
        weight = 2 if nP >= 2 else 1
        # More measurements where backup is present and primary coverage is high.
        if nP >= 2 and b1 >= 1:
            weight += 1
        weighted_cells.extend([cell] * weight)
    attempts = 0
    max_attempts = max(20000, n_subsets * 500)
    while len(unique) < n_subsets and attempts < max_attempts:
        attempts += 1
        nP, b0, b1, nE = weighted_cells[attempts % len(weighted_cells)]
        nB = int(rng.integers(b0, b1 + 1))
        if nP > len(groups['P']) or nB > len(groups['B']) or nE > len(groups['E']):
            continue
        idxs = []
        if nP:
            idxs += list(rng.choice(groups['P'], size=nP, replace=False))
        if nB:
            idxs += list(rng.choice(groups['B'], size=nB, replace=False))
        if nE:
            idxs += list(rng.choice(groups['E'], size=nE, replace=False))
        if not idxs:
            continue
        m = np.zeros(n, dtype=int)
        m[idxs] = 1
    ...
    # Preserve anchor order, then sort the rest for reproducibility.
    anchors = []
    seen = set()
    for m in masks:
        if m not in seen:
...
def build_design(mask_matrix: np.ndarray, head_records: Sequence[HeadRecord], model_name: str, ridge_pairs: bool = False):
    ...
    nP_total = max(1, sum(1 for h in head_records if h.group == 'P'))
    nB_total = max(1, sum(1 for h in head_records if h.group == 'B'))
    nE_total = max(1, sum(1 for h in head_records if h.group == 'E'))
    ...
    if model_name == 'additive_head':
        pass
    elif model_name == 'pb_occupancy_interaction':
        cols += ['P_B_occ']
    elif model_name == 'pb_count_interaction':
        cols += ['P_B_count']
    elif model_name == 'group_count_interaction':
        cols += ['P_B_count', 'P_E_count', 'B_E_count']
    ...
    for mask in mask_matrix:
        ...
        nP = float(g['P']); nB = float(g['B']); nE = float(g['E'])
        pfrac = nP / nP_total
        bfrac = nB / nB_total
        efrac = nE / nE_total
        feats = {'intercept': 1.0}
        ...
        feats['P_B_occ'] = float(g['P'] > 0 and g['B'] > 0)
        # Count interactions are the pre-registered Stage-2c terms.  They are
        # designed to ask whether self-repair grows with primary coverage and
        # backup coverage, instead of merely firing when both groups are present.
        feats['P_B_count'] = pfrac * bfrac
        feats['P_E_count'] = pfrac * efrac
        feats['B_E_count'] = bfrac * efrac
...
def annotate_prediction_rows(pred_df: pd.DataFrame, subset_df: pd.DataFrame):
    meta_cols = ['subset_idx', 'subset_name', 'n_heads', 'n_P', 'n_B', 'n_E', 'has_P', 'has_B', 'has_E', 'P_B', 'P_E', 'B_E', 'P_B_count', 'P_E_count', 'B_E_count', 'high_primary', 'full_primary']
...
def summarize_by_primary_count(pred_df: pd.DataFrame):
    rows = []
    for model, gm in pred_df.groupby('model'):
        for nP, g in gm.groupby('n_P'):
            rows.append({
                'model': model,
                'n_P': int(nP),
                'n': len(g),
        ...
        for nP, g1 in gm.groupby('n_P'):
            for hasB, g in g1.groupby('has_B'):
                rows.append({
                    'model': model,
                    'n_P': int(nP),
                    'has_B': int(hasB),
...
def paired_bootstrap_delta_summary(boot_df: pd.DataFrame, baseline: str = 'additive_head') -> pd.DataFrame:
    """Summarize paired bootstrap improvements over a baseline.

    Positive delta means the comparison model has lower MAE than the baseline:
        delta_MAE = MAE(baseline) - MAE(model)
    We gate success on the paired interval, not on a single point estimate.
    """
    rows = []
    pivot = boot_df.pivot_table(index='bootstrap', columns='model', values='mae', aggfunc='first')
    if baseline not in pivot.columns:
        return pd.DataFrame()
    for model in pivot.columns:
        if model == baseline:
            continue
        vals = (pivot[baseline] - pivot[model]).dropna().to_numpy(float)
        if len(vals) == 0:
            continue
        rows.append({
            'baseline': baseline,
            ...
            'delta_mae_mean': float(np.mean(vals)),
            'delta_mae_median': float(np.median(vals)),
            'delta_mae_std': float(np.std(vals, ddof=1)) if len(vals) > 1 else 0.0,
            'delta_mae_q05': float(np.quantile(vals, 0.05)),
            'delta_mae_q95': float(np.quantile(vals, 0.95)),
            'p_delta_gt_0': float(np.mean(vals > 0)),
            'strict_success_q05_gt_0': bool(np.quantile(vals, 0.05) > 0.0),
            'weak_success_p_gt_0_ge_0.95': bool(np.mean(vals > 0) >= 0.95),
        })
    return pd.DataFrame(rows)


def plot_paired_delta(delta_summary: pd.DataFrame, outpath: Path):
    import matplotlib.pyplot as plt
    if delta_summary.empty:
        return
    df = delta_summary.copy().sort_values('delta_mae_median', ascending=False)
    x = np.arange(len(df))
    ...
    ax.set_ylabel('paired ΔMAE vs additive (positive = better)')
    ax.set_title('Paired bootstrap improvement')
    fig.tight_layout()
    fig.savefig(outpath, dpi=180)
    plt.close(fig)


def plot_primary_coverage_errors(pred_df: pd.DataFrame, outpath: Path):
    import matplotlib.pyplot as plt
    # Show where interaction terms help/hurt as a function of primary coverage.
    rows = []
    for (model, nP), g in pred_df.groupby(['model', 'n_P']):
        rows.append({'model': model, 'n_P': int(nP), 'mae': float(np.mean(np.abs(g['error']))), 'n': len(g)})
    df = pd.DataFrame(rows)
    if df.empty:
        return
    ...
    for model, g in df.groupby('model'):
        g = g.sort_values('n_P')
        ax.plot(g['n_P'], g['mae'], marker='o', label=model)
    ax.set_xlabel('number of primary heads ablated (nP)')
    ax.set_ylabel('held-out MAE')
    ax.set_title('Errors by primary-group coverage')
...
def paired_delta_summary(boot_df: pd.DataFrame, baseline: str = 'additive_head') -> pd.DataFrame:
    """Paired bootstrap improvement over the additive baseline.

    Positive delta means the candidate has lower MAE than the baseline:
        delta_mae = MAE(baseline) - MAE(candidate)
    This is the criterion used for the improvement flags.  A point estimate is
    not enough; the q05 column should be above zero for a robust improvement.
    """
    pivot = boot_df.pivot_table(index='bootstrap', columns='model', values='mae', aggfunc='first')
    if baseline not in pivot.columns:
        return pd.DataFrame()
    ...
    for model in pivot.columns:
        if model == baseline:
            continue
        delta = pivot[baseline] - pivot[model]
        delta = delta.dropna().to_numpy(float)
        if len(delta) == 0:
            continue
        rows.append({
            'baseline': baseline,
            ...
            'frac_delta_gt_0': float(np.mean(delta > 0)),
            'robust_improvement': bool(np.quantile(delta, 0.05) > 0),
        })
    return pd.DataFrame(rows)
...
def plot_prediction_scatter(pred_df: pd.DataFrame, outpath: Path):
    ...
    markers = {'additive_head': 'o', 'pb_occupancy_interaction': 'P', 'pb_count_interaction': 's', 'group_count_interaction': '^', 'head_pair_sparse_ridge': 'x'}
    ...
    ax.set_title('IOI Stage 2c: head-level subset prediction')
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(outpath, dpi=180)
    plt.close(fig)
...
def plot_delta_mae(delta_summary: pd.DataFrame, outpath: Path):
    import matplotlib.pyplot as plt
    if delta_summary.empty:
        return
    df = delta_summary.copy().sort_values('delta_mae_median', ascending=False)
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(df['model'], rotation=25, ha='right')
    ax.set_ylabel('paired ΔMAE vs additive (positive is better)')
    ax.set_title('Paired bootstrap improvement over additive')
    fig.tight_layout()
    fig.savefig(outpath, dpi=180)
    plt.close(fig)
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='IOI Stage 2c: primary-stratified head-level subset-prediction benchmark')
    parser.add_argument('--outdir', type=str, default='runs/ioi_stage2c_head_subset')
    ...
    parser.add_argument('--n-subsets', type=int, default=240)
    parser.add_argument('--sampling-mode', type=str, default='primary_stratified', choices=['primary_stratified', 'random'], help='Subset design. primary_stratified is the Stage-2c default.')
    parser.add_argument('--p-include', type=float, default=0.35, help='Ignored by the primary-stratified sampler except for compatibility')
    ...
    pd.DataFrame([asdict(h) for h in head_records]).to_csv(outdir / 'ioi_stage2c_head_records.csv', index=False)
    ...
    pd.DataFrame([asdict(r) for r in records]).to_csv(outdir / 'ioi_stage2c_prompts.csv', index=False)

    mean_z = compute_mean_z(model, ref_tokens, args.batch_size) if args.ablation == 'mean' else None


    if args.sampling_mode == 'primary_stratified':
        mask_matrix = sample_primary_stratified_masks(head_records, n_subsets=args.n_subsets, seed=args.seed + 333)
    else:
    ...
    for idx, mask in enumerate(mask_matrix):
        ...
        subset_rows.append({
            ...
            'P_B_count': float((g['P'] / max(1, len([h for h in head_records if h.group == 'P']))) * (g['B'] / max(1, len([h for h in head_records if h.group == 'B'])))),
            'P_E_count': float((g['P'] / max(1, len([h for h in head_records if h.group == 'P']))) * (g['E'] / max(1, len([h for h in head_records if h.group == 'E'])))),
            'B_E_count': float((g['B'] / max(1, len([h for h in head_records if h.group == 'B']))) * (g['E'] / max(1, len([h for h in head_records if h.group == 'E'])))),
            'high_primary': int(g['P'] >= 2),
            'full_primary': int(g['P'] == len([h for h in head_records if h.group == 'P'])),
        })
    subset_df = pd.DataFrame(subset_rows)
    subset_df.to_csv(outdir / 'ioi_stage2c_subset_design.csv', index=False)
    ...
    meas_df.to_csv(outdir / 'ioi_stage2c_subset_measurements.csv', index=False)
    ...
    pd.DataFrame(long_rows).to_csv(outdir / 'ioi_stage2c_per_prompt_drops.csv', index=False)

    models = ['additive_head', 'pb_occupancy_interaction', 'pb_count_interaction', 'group_count_interaction']
    ...
    primary_count_summary = summarize_by_primary_count(pred_df)

    pred_df.to_csv(outdir / 'ioi_stage2c_kfold_predictions.csv', index=False)
    fit_summary.to_csv(outdir / 'ioi_stage2c_fit_summary.csv', index=False)
    coef_df.to_csv(outdir / 'ioi_stage2c_coefficients.csv', index=False)
    occupancy_summary.to_csv(outdir / 'ioi_stage2c_group_occupancy_errors.csv', index=False)
    primary_count_summary.to_csv(outdir / 'ioi_stage2c_primary_count_errors.csv', index=False)
    ...
    boot_df.to_csv(outdir / 'ioi_stage2c_bootstrap_metrics.csv', index=False)
    boot_summary.to_csv(outdir / 'ioi_stage2c_bootstrap_summary.csv', index=False)
    paired_delta = paired_bootstrap_delta_summary(boot_df, baseline='additive_head')
    paired_delta.to_csv(outdir / 'ioi_stage2c_paired_delta_mae.csv', index=False)
    ...
    diag['pb_occupancy_interaction_mae'] = get_mae('pb_occupancy_interaction')
    diag['pb_count_interaction_mae'] = get_mae('pb_count_interaction')
    diag['group_count_interaction_mae'] = get_mae('group_count_interaction')
    # Improvement flags are filled from paired bootstrap CIs below, not from point estimates.
    diag['mae_reduction_group_count_vs_additive_point'] = float((diag['additive_head_mae'] - diag['group_count_interaction_mae']) / max(diag['additive_head_mae'], 1e-12))
    # Robust success is based on paired bootstrap confidence intervals.
    for _, row in paired_delta.iterrows():
        m = str(row['model'])
        diag[f'{m}_delta_mae_mean_vs_additive'] = float(row['delta_mae_mean'])
        diag[f'{m}_delta_mae_q05_vs_additive'] = float(row['delta_mae_q05'])
        diag[f'{m}_p_delta_gt_0_vs_additive'] = float(row['p_delta_gt_0'])
        diag[f'{m}_strict_success_q05_gt_0'] = bool(row['strict_success_q05_gt_0'])
        diag[f'{m}_weak_success_p_gt_0_ge_0.95'] = bool(row['weak_success_p_gt_0_ge_0.95'])

    with open(outdir / 'ioi_stage2c_diagnostics.json', 'w') as f:
    ...
    metadata = {
        ...
        'primary_comparison': 'per-head additive observer vs count/group-interaction observers on primary-stratified held-out head subsets; success gated by paired bootstrap ΔMAE, not point estimates',
        'note': 'This is the single pre-registered Stage-2c variant after the Stage-2b null: primary-stratified subset sampling toward high-primary-coverage self-repair regimes.',
    }
    with open(outdir / 'ioi_stage2c_metadata.json', 'w') as f:
        json.dump(metadata, f, indent=2)

    plot_prediction_scatter(pred_df, outdir / 'ioi_stage2c_prediction_scatter.png')
    plot_mae_bar(fit_summary, outdir / 'ioi_stage2c_mae_bar.png')
    plot_bootstrap_mae(boot_summary, outdir / 'ioi_stage2c_bootstrap_mae.png')
    plot_paired_delta(paired_delta, outdir / 'ioi_stage2c_paired_delta_mae.png')
    plot_primary_coverage_errors(pred_df, outdir / 'ioi_stage2c_primary_coverage_errors.png')

    # Markdown report.
    report = []
    report.append('# IOI Stage 2c head-level subset-prediction report\n')
    report.append('This is the pre-registered Stage-2c variant: primary-stratified random subsets of individual heads. It tests whether interaction terms help when the subset design actually contains the high-primary-coverage regimes where IOI self-repair is mechanistically predicted to matter.\n')
    report.append('## Setup\n')
    ...
    report.append(paired_delta.to_markdown(index=False))
    ...
    report.append(primary_count_summary.to_markdown(index=False))
    (outdir / 'ioi_stage2c_report.md').write_text('\n'.join(report))

    print('\n=== IOI Stage 2c fit summary ===')
    print(fit_summary.to_string(index=False))
    print('\n=== Bootstrap summary ===')
    print(boot_summary.to_string(index=False))
    print('\n=== Paired bootstrap ΔMAE vs additive ===')
    print(paired_delta.to_string(index=False))
    print('\n=== Diagnostics ===')
